...
//        break;
  ...
  private P3 centerPoint, spherePoint;
...
//        htParams.put("fixedElement", type);
  ...
  private void connectAtoms(float bd, int bondOrder, BS bs1, BS bs2) {
    vwr.makeConnections((bd - 0.01f), (bd + 0.01f), bondOrder, T.modifyorcreate, bs1, bs2, new BS(), false, false, 0);
  }
   ...
   */
  public int cmdAssignAddAtoms(String type, P3[] pts, BS bsAtoms, String packing,
    ...
    try {
      ...
      if (isPoint) {
        ...
        for (int i = 0; i < pts.length; i++) {
          assignAtoms(P3.newP(pts[i]), true, atomIndex, stype, null, false,
        ...
        for (int i = bsAtoms.nextSetBit(0); i >= 0; i = bsAtoms
          ...
          assignAtoms(P3.newP(a), false, -1, stype, null, false, null,
  ...
  public int cmdAssignMoveAtoms(BS bsSelected, BS bsFixed, BS bsModelAtoms, int iatom, P3 p, P3[] pts, boolean allowProjection) {
    ...
    if ((n = moveConstrained(iatom, bsFixed, bsModelAtoms, p, !isOccSet, allowProjection)) == 0 || Float.isNaN(p.x)
        || !isOccSet) {
...
//        m = sg.getTransform(fa, fb, true);
   ...
   * @return number of atoms moved
   */
  public int moveConstrained(int iatom, BS bsFixed, BS bsModelAtoms, P3 ptNew,
      ...
      if (c.type == Constraint.TYPE_LOCKED) {
        ...
        if (a != b) {
          ...
          if (m == null) {
            ...
            iatom = b.i;
            P3 p = P3.new3(ptNew.x, ptNew.y, ptNew.z);
            sym.toFractional(p, true);
            m.rotTrans(p);
  ...
  public int cmdRotateAtoms(BS bsAtoms, P3[] points, float endDegrees) {
    ...
    P3[] apos0 = new P3[vwr.ms.at.length];
