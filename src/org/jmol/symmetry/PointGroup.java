...
/*
 * 2023.11.22 adds Hermann-Mauguin names as info.hmName and in parentheses in reports after 
 * Bob Hanson 7/2008
 * 
  ...
  Object getInfo(int modelIndex, String drawID, boolean asInfo, String type,
    ...
    if (asDraw) {
        ...
        if (!haveType || type.equalsIgnoreCase(label)
          ...
          for (int j = 0; j < nAxes[i]; j++) {
            ...
            if (op.type == OPERATION_IMPROPER_AXIS)
              scale = -scale;
            sb.append(drawID + "pgva").append(m).append(label).append("_")
                .appendI(j + 1).append(" width 0.05 scale ").appendF(scale)
      ...
      if (!haveType || type.equalsIgnoreCase("Cs")) {
        for (int j = 0; j < nAxes[0]; j++) {
          ...
          sb.append(drawID + "pgvp").append(m).appendI(j + 1)
              .append("disk scale ").appendF(scaleFactor * radius * 2)
          ...
          sb.append(drawID + "pgvp").append(m).appendI(j + 1)
              ...
              .appendF(scaleFactor * radius * 2).append(" arc ")
              .append(Escape.eP(v));
          v.scaleAdd2(-2, op.normalOrAxis, v);
      ...
      for (int i = maxAxis; --i >= 2;)
        if (nAxes[i] > 0) {
          sb.append(" n").append(i < firstProper ? "S" : "C")
              .appendI(i % firstProper);
    ...
    for (int i = maxAxis; --i >= 0;) {
      if (nAxes[i] > 0) {
        ...
        if (asInfo)
          ...
          sb.append("\n\n").append(name).append("\tn").append(label)
              .append("\t").appendI(nAxes[i]).append("\t").appendI(n);
        n *= nAxes[i];
        nTotal += n;
        nElements += nAxes[i];
        ...
        for (int j = 0; j < nAxes[i]; j++) {
          ...
          if (asInfo) {
            ...
            sb.append("\n").append(name).append("\t").append(label).append("_")
    ...
    if (!asInfo) {
      ...
      sb.append("\n").append(name).append("\tCi\t  ").appendI(n).append("\t  ")
          .appendI(n);
