...
public class SpaceGroupFinder {
  ...
  public Object findSpaceGroup(Viewer vwr, BS atoms0, String xyzList,
                               float[] unitCellParams, SymmetryInterface uc,
  ...
  private static SymmetryInterface createCompatibleUnitCell(SpaceGroup sg,
    ...
    sym = new Symmetry().setUnitCell(new float[] { a, b, c, alpha, beta, gamma }, false);
    sym.setSpaceGroupTo(sg);    
    return sym;
  }

  private static boolean isHexagonal(int n, float[] params) {
...
//  }
  ...
  private static boolean approx0(float f) {
    return (Math.abs(f) < SLOP001);
  }

  private static boolean approx000(float f) {
    return (Math.abs(f) < SLOP0001);
  }

  private static boolean approx0014(float f) {
...
//  }
//
  private static int approxInt(float finv) {
