...
public class CmdExt extends ScriptExt {
  ...
  public Object getBitsetIdentFull(BS bs, String label,
    ...
    LabelToken[] tokens = (asIdentity ? null : isAtoms ? LabelToken.compile(
        vwr, label, '\0', null) : LabelToken.compile(vwr, label, '\1',
        htValues));
    ...
    for (int j = (haveIndex ? index : bs.nextSetBit(0)); j >= 0; j = bs
      ...
      if (isAtoms) {
        if (asIdentity)
          ...
          str = LabelToken.formatLabelAtomArray(vwr, modelSet.at[j], tokens,
              '\0', indices, ptTemp);
      } else {
  ...
  private void assign() throws ScriptException {
    ...
    if (isAdd) {
      ...
      case T.wyckoff:
        ...
        char w = (wyckoff.length() > 1 || wyckoff.length() == 0 ? '\0'
            : wyckoff.charAt(0));
        if (w < 'a' && w != 'A' || w > 'z')
          invArg();
    ...
    case T.atoms:
      e.clearDefinedVariableAtomSets();
      vwr.getModelkit(false).cmdAssignAtom(index, pt, type, e.fullCommand,
          isClick);
      break;
    case T.bonds:
    ...
    case T.moveto:
      ...
      int nm = vwr.getModelkit(false).cmdAssignMoveAtoms(bs, null, null, index,
          pt, pts, true);
      if (nm > 0)
